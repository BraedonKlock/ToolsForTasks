...
* It contains an arrayList of Job objects and each Job can contain an arrayList of Tools and Materials.
* 
* This class allows users to:
* - Add, edit, delete, list, and search for jobs
* 
* - it also manages each job's associated tools and materials including:
* 	adding, editing, and deleting individual tool's/material's and their attributes from a Job object
* 
...
public class JobList {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        if (quantity < 1) throw new NumberFormatException(); // User must input number greater than 0
                        break;
    ...
    while (true) {
        ...
        if (response.equals("no")) {
        	...
        	while (true) {
        	    System.out.println("\nMaterial name (or type \"Exit\" to stop adding materials): ");
        	    String materialName = scanner.nextLine();
        	    if (materialName.equalsIgnoreCase("Exit")) break;

        	    String quantityStr;

        	    // While Loopt to control flow and user input
        	    while (true) {
        	        System.out.print("\nQuantity (Doesn't have to be a number - e.g., '300 feet'): ");
        	        quantityStr = scanner.nextLine();

        	        if (quantityStr.equalsIgnoreCase("Exit")) break;

        	        if (quantityStr.trim().isEmpty()) {
        	            System.out.println("\n***Quantity cannot be empty. Please enter a value.***\n");
        	        } else {
        	            break; // valid input, break out of loop
        	        }
        	    }

        	    // Add the material with the string quantity
        	    job.addMaterial(new Materials(materialName, quantityStr));
        	}
        }
